...
The production site uses Cloudflare Web Analytics. Cloudflare Pages automatically injects a performance beacon loaded from `static.cloudflareinsights.com`; it reports to the same site's `/cdn-cgi/rum` endpoint. The resulting dashboard contains aggregate visits, page views, paths, referrers, country, broad device/browser/operating-system information, navigation type, and page-performance metrics.
...
本番サイトではCloudflare Web Analyticsを使用します。Cloudflare Pagesが`static.cloudflareinsights.com`からperformance beaconを自動挿入し、同一siteの`/cdn-cgi/rum` endpointへ送信します。dashboardでは、集計された訪問、page view、path、参照元、国、端末・browser・OSの大まかな情報、navigation type、ページ表示性能を確認できます。
