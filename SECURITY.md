...
Anki Workload Planner is a static, browser-only application. It has no project-operated backend, user account, database, cloud sync, Anki integration, or AI API. Planner inputs and snapshots are stored in browser `localStorage`, and calculations and exports run in the browser. The production site uses Cloudflare Web Analytics for aggregate page-view and performance metrics, but the beacon does not read browser storage and the application does not send planner data to it.
...
- Keep third-party scripts, analytics, remote fonts, and runtime CDN dependencies disabled unless they are reviewed, narrowly allowed by CSP, and accurately disclosed. The production deployment permits only Cloudflare's automatically injected Web Analytics beacon.
...
Anki負荷プランナーは、ブラウザだけで動く静的アプリです。プロジェクトが運用するbackend、ユーザーアカウント、database、cloud sync、Anki連携、AI APIはありません。計画の入力値とスナップショットはbrowserの`localStorage`に保存され、計算と書き出しもブラウザ内で行われます。本番siteでは、集計されたpage viewと表示性能を確認するためCloudflare Web Analyticsを使用しますが、beaconはbrowser storageを読み取らず、アプリも計画データを送信しません。
...
- third-party script、analytics、外部font、実行時CDN dependencyは、reviewし、CSPで狭く許可し、利用者へ正確に開示した場合にだけ有効化する。本番deploymentで許可するのは、Cloudflareが自動挿入するWeb Analytics beaconだけです。
